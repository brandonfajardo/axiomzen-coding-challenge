...
export const findRepo = (user, repoName, currentPageNumber) => dispatch => {
  ...
  axios
    .get(
      `https://api.github.com/repos/${user}/${repoName}/issues?page=${currentPageNumber}&state=all`
    )
    .then(({ data, headers }) => {
    ...
    .then(repo => {
      ...
      if (data.length) {
        setTimeout(() => {
          // Responding too quickly so I added 1s of delay
          dispatch({
            type: FIND_REPO_SUCCESS,
            payload: { data, user, repoName, currentPageNumber, lastPageNumber }
          });
          history.push(
            `/issues?user=${user}&repoName=${repoName}&page=${currentPageNumber}`
          );
        }, 1000);
      }
    })
    .catch(e => {
      dispatch({
        type: FIND_REPO_FAILURE,
        payload:
          "The following repo does not have any issues or there was an error in your request."
      });
    });
};
